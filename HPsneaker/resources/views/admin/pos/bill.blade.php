            ...
            <div class="card shadow-sm">
                ...
                <div class="card-body">
                    ...
                    <div class="mt-3 text-end">
                        ...
                        <div><strong>Giảm giá:</strong> {{ number_format($order->discount_applied, 0, ',', '.') }} VNĐ</div>
                        <div><strong>Thanh toán:</strong> {{ number_format($order->total_amount - $order->discount_applied, 0, ',', '.') }} VNĐ</div>
                        <div><strong>Phương thức:</strong> {{ $order->payment_method }}</div>
                        <div><strong>Trạng thái:</strong> {{ $order->status }}</div>
                    </div>
                </div>
                <div class="card-footer text-center">
                    <a href="{{ route('pos.index') }}" class="btn btn-primary">Quay lại POS</a>
                </div>
            </div>
        </div>
    </div>
